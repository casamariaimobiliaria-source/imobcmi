...
export const Finance = () => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      // Logic for Multiple Records (Installments or Recurrence)
      if (isInstallment && installmentCount > 1) {
        const baseAmount = formData.amount / installmentCount;
        const baseDate = new Date(formData.dueDate || new Date().toISOString());
        ...
        for (let i = 0; i < installmentCount; i++) {
          ...
          addFinancialRecord({
            ...
            date: formData.date // Competence date remains the same or could shift too, usually keeps same for accrual
          } as FinancialRecord);
        }
      } else if (isRecurrent && recurrenceCount > 1) {
        const baseDate = new Date(formData.dueDate || new Date().toISOString());
        ...
        for (let i = 0; i < recurrenceCount; i++) {
          ...
          addFinancialRecord({
            ...
            id: generateId(),
            dueDate: newDate.toISOString().split('T')[0],
            // Competence date usually shifts for recurrence (e.g. Rent Jan, Rent Feb)
  ...
  const StatusBadge = ({ status }: { status: FinanceStatus }) => (
    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${status === 'paid' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
      }`}>
              ...
              {filteredRecords
                ...
                .map(record => (
                  <tr key={record.id} className="hover:bg-slate-50">
                    ...
                    <td className={`px-6 py-4 text-right font-bold ${record.type === 'income' ? 'text-emerald-600' : 'text-red-600'
                      }`}>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              {/* Advanced Options (Only for New Records) */}
              {!editingId && (
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 space-y-3">
                  <div className="flex items-center gap-4">
                    <label className="flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
                      <input
                        ...
                        onChange={(e) => { setIsInstallment(e.target.checked); if (e.target.checked) setIsRecurrent(false); }}
                    ...
                    <label className="flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={isRecurrent}
                        onChange={(e) => { setIsRecurrent(e.target.checked); if (e.target.checked) setIsInstallment(false); }}
                        className="rounded text-blue-600 focus:ring-blue-500"
                      />
                      Recorrência?
                    </label>
                  ...
                  {isInstallment && (
                    <div className="flex items-center gap-2 animate-in slide-in-from-top-1">
                      <label className="text-xs font-semibold text-slate-500">Nº Parcelas:</label>
                      <input
                        type="number" min="2" max="60"
                        value={installmentCount}
                        onChange={(e) => setInstallmentCount(parseInt(e.target.value))}
                        className="w-20 p-1.5 text-sm border border-slate-300 rounded"
                      />
                      <span className="text-xs text-slate-400">
                        (Valor da parcela: {formatCurrency((formData.amount || 0) / installmentCount)})
                      </span>
                    </div>
                  )}
                  ...
                  {isRecurrent && (
                    <div className="flex items-center gap-2 animate-in slide-in-from-top-1">
                      <label className="text-xs font-semibold text-slate-500">Repetir por (meses):</label>
                      <input
