...
export const Dashboard = () => {
    ...
    const CustomTooltip = ({ active, payload, label }: any) => {
        if (active && payload && payload.length) {
            return (
                <div className="premium-card p-4 !rounded-xl !border-border shadow-2xl backdrop-blur-md">
                    <p className="text-foreground font-black mb-3 border-b border-border/50 pb-2 text-xs uppercase tracking-widest">{selectedMonth ? `Dia ${label}` : label}</p>
                    {payload.map((entry: any, index: number) => (
                        <div key={index} className="flex justify-between items-center gap-6 mb-1.5">
                            ...
                            <span className="font-black text-xs" style={{ color: entry.stroke || entry.fill }}>
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between w-full">
                    ...
                    <div className="flex gap-4">
                        <button
                            ...
                            className={`flex items-center gap-2 px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all mt-4 md:mt-0 ${isAnalyzing
                                ? 'bg-indigo-500/20 border-indigo-500/40 text-indigo-400 animate-pulse'
                                : 'bg-indigo-500/10 border-indigo-500/20 text-indigo-400 hover:bg-indigo-500/20 hover:border-indigo-500/40 shadow-lg shadow-indigo-500/10'
                                }`}
                        >
                            <BrainCircuit size={18} />
                            {isAnalyzing ? 'Analisando Saúde Financeira...' : 'Diagnóstico Financeiro IA'}
                        </button>
                    </div>
                </div>

                {/* Filters Panel - Dark Modern */}
                <div className="premium-card p-2 flex flex-wrap gap-2 items-center !rounded-2xl !border-white/10 w-full xl:w-auto overflow-x-auto custom-scrollbar no-scrollbar">
                    <div className="flex items-center gap-2 px-3 md:px-4 py-2 bg-secondary rounded-xl text-primary border border-border flex-shrink-0">
                        <Filter size={14} className="md:size-[16px]" />
                        <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest whitespace-nowrap">Filtros</span>
                    </div>
            ...
            <style>{`
                h1 span, h2 span, h3 span { @apply text-primary drop-shadow-[0_0_15px_rgba(6,182,212,0.3)]; }
            `}</style>
        </div>
    );
};
