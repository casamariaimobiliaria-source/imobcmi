    ...
    return (
        <form onSubmit={onSubmit} className="space-y-8 animate-in slide-in-from-right-4 fade-in duration-300 pb-20">
            <Card className="p-6 md:p-8 space-y-8">
                <div>
                    <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-4 border-b border-white/5 pb-2 italic">Dados do Imóvel</h3>
                ...
                <div>
                    <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-4 border-b border-white/5 pb-2 italic">Envolvidos</h3>
                ...
                <div>
                    <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-4 border-b border-white/5 pb-2 italic">Comissionamento (Split)</h3>
                        ...
                        <div className="bg-red-500/5 rounded-2xl border border-red-500/10 p-5 mb-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="flex items-end gap-3">
                                    <Input type="number" step="0.1" label="% Nota Fiscal (Imposto)" className="!bg-red-500/5 !border-red-500/20 text-red-400" value={formData.taxPercent || ''} onChange={e => setFormData({ ...formData, taxPercent: parseFloat(e.target.value) })} />
                                    <div className="mb-3 text-xs text-red-500 font-black whitespace-nowrap italic">= - {formatCurrency(formData.taxValue || 0)}</div>
                                </div>
                                <div className="flex gap-3">
                                    <Input label="Descontos Diversos (Descrição)" placeholder="Ex: Taxa WayBropay" className="!bg-red-500/5 !border-red-500/20" value={formData.miscExpensesDescription || ''} onChange={e => setFormData({ ...formData, miscExpensesDescription: e.target.value })} />
                                    <Input type="number" step="0.01" label="Valor (R$)" className="w-32 !bg-red-500/5 !border-red-500/20 text-red-400" value={formData.miscExpensesValue || ''} onChange={e => setFormData({ ...formData, miscExpensesValue: parseFloat(e.target.value) })} />
                                </div>
                            </div>
                        </div>
                    ) : (
                    ...
                    <div className="mb-10 px-2 group">
                        <label className="block text-[10px] font-black text-slate-500 mb-4 uppercase tracking-widest italic">Divisão (Split)</label>
                        <input type="range" min="0" max="100" step="1" disabled={isReadOnly} className="w-full h-1.5 rounded-lg appearance-none cursor-pointer mb-4 bg-slate-800 accent-cyan-500" value={formData.agentSplitPercent || 0} onChange={e => setFormData({ ...formData, agentSplitPercent: parseFloat(e.target.value) })} />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-10">
                        <Card className="!bg-white/[0.02] border-white/5 shadow-none group hover:!bg-white/[0.04]">
                            <div className="p-5">
                                <div className="flex items-center gap-2 mb-4 text-slate-500 font-black uppercase text-[10px] tracking-widest italic border-b border-white/5 pb-2">
                                    <Calculator size={16} /> Base de Cálculo
                                </div>
                                <div className="space-y-2 text-xs font-bold">
                                    <div className="flex justify-between text-slate-400"><span>(+) Bruto</span><span className="text-slate-200">{formatCurrency(formData.grossCommission || 0)}</span></div>
                                    <div className="flex justify-between text-red-500/70"><span>(-) Impostos</span><span>{formatCurrency(formData.taxValue || 0)}</span></div>
                                    {formData.miscExpensesValue ? <div className="flex justify-between text-red-500/70"><span>(-) Diversos</span><span>{formatCurrency(formData.miscExpensesValue)}</span></div> : null}
                                    <div className="border-t border-white/10 pt-3 mt-3 flex justify-between font-black text-white text-sm italic"><span>(=) Líquido</span><span>{formatCurrency(baseCalculation)}</span></div>
                                </div>
                            </div>
                        </Card>
                        <Card className="!bg-indigo-500/[0.03] border-indigo-500/10 shadow-none hover:!bg-indigo-500/[0.06]">
                            <div className="p-5">
                                <div className="flex items-center gap-2 mb-4 text-indigo-400/70 font-black uppercase text-[10px] tracking-widest italic border-b border-indigo-500/10 pb-2"><Building2 size={16} /> Imobiliária</div>
                                <h3 className="text-3xl font-black text-white italic tracking-tighter mb-4">{formatCurrency(formData.agencyCommission || 0)}</h3>
                                <div className="text-[10px] font-black uppercase tracking-widest text-indigo-300/50 bg-indigo-500/10 p-2 rounded-lg text-center">Participação: {100 - (formData.agentSplitPercent || 0)}%</div>
                            </div>
                        </Card>
                        <Card className="!bg-emerald-500/[0.03] border-emerald-500/10 shadow-none ring-1 ring-emerald-500/20 hover:!bg-emerald-500/[0.06]">
                            <div className="p-5">
                                <div className="flex items-center gap-2 mb-4 text-emerald-400/70 font-black uppercase text-[10px] tracking-widest italic border-b border-emerald-500/10 pb-2"><UserIcon size={16} /> Corretor</div>
                                <h3 className="text-3xl font-black text-white italic tracking-tighter mb-4">{formatCurrency(formData.agentCommission || 0)}</h3>
                                <div className="space-y-2">
                                    <div className="text-[10px] font-black uppercase tracking-widest text-emerald-300/50 bg-emerald-500/10 p-2 rounded-lg text-center">Participação: {formData.agentSplitPercent}%</div>
                                    <div className="text-[9px] font-bold text-center text-emerald-500/40 uppercase italic">Dedução Proporcional: {formatCurrency(agentDeductionShare)}</div>
                                </div>
                            </div>
                        </Card>
                    </div>

                    <div className="mt-10 pt-6 border-t border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div className="flex-1">
                            ...
                            {user?.role === 'admin' ? (
                                <select className="w-full md:w-48 p-3 rounded-xl border border-white/10 bg-[#09090b] text-white focus:ring-2 focus:ring-cyan-500/40 outline-none uppercase text-[10px] font-black italic tracking-widest" value={formData.status} onChange={e => setFormData({ ...formData, status: e.target.value as any })}>
