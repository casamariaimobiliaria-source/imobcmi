...
export const Reports = () => {
    ...
    const dreData = useMemo(() => {
        ...
        return [{ name: 'Receitas', value: income }, { name: 'Despesas', value: expense }];
    }, [filteredFinance]);
        ...
        if (selectedReport?.id === 'dre') return (
            <ResponsiveContainer width="100%" height={350}>
                <BarChart data={dreData}>
                    ...
                    <Tooltip contentStyle={{ backgroundColor: '#0f0f12', border: '1px solid #333', borderRadius: '12px' }} />
                    <Bar dataKey="value" barSize={40}>
                        {dreData.map((entry, index) => <Cell key={index} fill={entry.name === 'Receitas' ? '#10b981' : '#f43f5e'} />)}
                    </Bar>
                </BarChart>
            </ResponsiveContainer>
        );
    ...
    return (
        <div className="space-y-8 pb-12 animate-in fade-in duration-700">
            ...
            <style>{`
                .report-input { @apply bg-[#050505] border border-white/5 rounded-2xl pl-16 pr-6 py-4 text-white text-[11px] outline-none focus:border-blue-500/50 transition-all font-bold shadow-inner placeholder:text-slate-800; }
                .custom-scrollbar::-webkit-scrollbar { width: 4px; }
                .custom-scrollbar::-webkit-scrollbar-thumb { background: #1e1e2d; border-radius: 10px; }
                .printable-card { font-family: 'Inter', sans-serif; background: #fff !important; color: #000 !important; }
                .printable-card * { color: inherit !important; }
                .printable-card .text-blue-600 { color: #2563eb !important; }
                .printable-card .text-emerald-600 { color: #059669 !important; }
                .printable-card .bg-zinc-50, .printable-card .bg-slate-50 { background-color: #f8fafc !important; }
            `}</style>
        </div>
    );
};
