...
export const Finance = () => {
  ...
  return (
    <div className="space-y-8 pb-12 animate-in fade-in duration-700">
      ...
      {/* CRUD MODAL */}
      {showModal && (
        <div className="fixed inset-0 bg-background/90 backdrop-blur-2xl z-[70] flex items-center justify-center p-4">
          <div className="premium-card !bg-background border border-white/10 w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="px-10 py-8 bg-secondary/30 border-b border-white/10 flex justify-between items-center relative">
            ...
            <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-10 space-y-8 custom-scrollbar">
              ...
              <div className="space-y-6">
                ...
                {formData.category === 'Comissão' && (
                  <div className="bg-indigo-500/5 border border-indigo-500/20 p-5 rounded-3xl animate-in fade-in zoom-in-95 duration-300">
                    <label className="text-[10px] font-black text-indigo-400 uppercase tracking-widest block mb-3">Vincular Beneficiário</label>
                    <select className="finance-input w-full bg-black/40" value={formData.relatedEntityId || ''} onChange={e => setFormData({ ...formData, relatedEntityId: e.target.value })}>
                      <option value="">Selecione...</option>
      ...
      <style>{`
        .finance-input { @apply bg-[#050505] border border-white/10 rounded-2xl px-5 py-4 text-foreground outline-none focus:border-cyan-500/50 shadow-inner placeholder:text-muted-foreground/30 transition-all; }
